...
class MongoDBWrapper:
    ...
    def __init__(self, uri, db_name):
        ...
        self.admins = None
        self.temp_gods = None  # NEW: Collection for Temporary Gods
    ...
    async def connect(self):
        ...
        self.temp_gods = self.db.temp_gods  # NEW: Temporary Gods collection
    ...
    # ==========================================
    # TEMPORARY GOD METHODS (NEW)
    # ==========================================

    async def add_temp_god(self, user_id: int) -> bool:
        """
        Promote a user to Temporary God status.
        
        Temporary Gods have all permissions (like Permanent God).
        
        Args:
            user_id (int): Discord user ID to promote
        ...
        try:
            ...
            existing = await self.temp_gods.find_one({"user_id": user_id})
            if existing:
                return True
            
            await self.temp_gods.insert_one({
                "user_id": user_id,
    ...
    async def remove_temp_god(self, user_id: int) -> bool:
        """
        Demote a user from Temporary God status.
        
        Args:
            user_id (int): Discord user ID to demote
            
        Returns:
            bool: True if successful, False otherwise
        """
        try:
            result = await self.temp_gods.delete_one({"user_id": user_id})
    ...
    async def is_temp_god(self, user_id: int) -> bool:
        """
        ...
        """
        try:
            result = await self.temp_gods.find_one({"user_id": user_id})
            return result is not None
        except Exception as e:
            print(f"[ERROR] is_temp_god failed for {user_id}: {e}")
            return False

    async def get_all_temp_gods(self) -> list:
        """
