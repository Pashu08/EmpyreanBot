...
async def log_admin_command(bot, ctx, command_name, details=""):
    ...
    try:
        ...
        print(f"[DEBUG] admin: Failed to send log to admin channel: {e}")
...
class Admin(commands.Cog):
    ...
    async def allow(self, ctx, member: discord.Member, permission: str):
        if ctx.author.id != PERMANENT_GOD:
            await ctx.send("❌ Only the Permanent God can grant permissions.", delete_after=2)
            return
        ...
        if permission.lower() == "all":
            ...
            await ctx.send(f"✅ {member.mention} granted **ALL** permissions.", delete_after=2)
        else:
            await add_permission(self.bot, member.id, permission.lower())
            await ctx.send(f"✅ {member.mention} granted `{permission}` permission.", delete_after=2)
    ...
    async def deny(self, ctx, member: discord.Member, permission: str):
        if ctx.author.id != PERMANENT_GOD:
            await ctx.send("❌ Only the Permanent God can remove permissions.", delete_after=2)
            return
        await ctx.message.delete()
        ...
        if permission.lower() == "all":
            ...
            await ctx.send(f"✅ Removed **ALL** permissions from {member.mention}.", delete_after=2)
        else:
            await remove_permission(self.bot, member.id, permission.lower())
            await ctx.send(f"✅ Removed `{permission}` permission from {member.mention}.", delete_after=2)
    ...
    async def perms(self, ctx, member: discord.Member = None):
        if not await has_permission(self.bot, ctx.author.id, "system"):
            if ctx.author.id != PERMANENT_GOD:
                await ctx.send("❌ You don't have permission to view permissions.", delete_after=2)
                return
        await ctx.message.delete()
    ...
    # ==========================================
    # DIVINE (Admin help menu - IMPROVED)
    # ==========================================
    @commands.command()
    async def divine(self, ctx):
        ...
        try:
            ...
            await ctx.send("❌ Open your DMs to receive the scroll.", delete_after=2)
    ...
    async def pulse(self, ctx):
        ...
        if mechanics_cog:
            ...
            await ctx.send("🌀 **Divine Pulse:** Recovery triggered.", delete_after=2)
    ...
    async def promote(self, ctx, member: discord.Member):
        if ctx.author.id != PERMANENT_GOD:
            await ctx.send("❌ Only the Permanent God can promote.", delete_after=2)
            return
        await ctx.message.delete()
        ...
        await ctx.send(f"🌟 {member.mention} promoted to Temporary God.", delete_after=2)

    @commands.command()
    async def demote(self, ctx, member: discord.Member):
        if ctx.author.id != PERMANENT_GOD:
            await ctx.send("❌ Only the Permanent God can demote.", delete_after=2)
            return
        await ctx.message.delete()
        ...
        if member.id in temporary_gods:
            ...
            await ctx.send(f"💢 {member.mention} stripped of Divine Power.", delete_after=2)
        else:
            await ctx.send(f"❌ {member.mention} is not a God.", delete_after=2)
    ...
    async def ban(self, ctx, member: discord.Member, *, reason: str = "No reason provided"):
        if not await has_permission(self.bot, ctx.author.id, "system"):
            await ctx.send("❌ You don't have permission to ban users.", delete_after=2)
            return
        await ctx.message.delete()
    ...
    async def unban(self, ctx, member: discord.Member):
        if not await has_permission(self.bot, ctx.author.id, "system"):
            await ctx.send("❌ You don't have permission to unban users.", delete_after=2)
            return
        await ctx.message.delete()
    ...
    async def toggle(self, ctx, feature: str):
        ...
        await ctx.send(f"✅ {feature.capitalize()} is now **{status}**.", delete_after=2)
    ...
    async def set_cooldown(self, ctx, command: str, seconds: int):
        ...
        await ctx.send(f"⏳ Cooldown for `!{command}` set to **{seconds} seconds**.", delete_after=2)
    ...
    async def set_emoji(self, ctx, name: str, emoji: str):
        ...
        await ctx.send(f"✅ Emoji `{name}` set to {emoji}.", delete_after=2)
    ...
    async def set_message(self, ctx, key: str, *, text: str):
        ...
        await ctx.send(f"✅ Message `{key}` updated.", delete_after=2)
    ...
    async def debug(self, ctx, state: str):
        ...
        if state.lower() not in ["on", "off"]:
            await ctx.send("❌ Use `!debug on` or `!debug off`.", delete_after=2)
            return
        ...
        await ctx.send(f"🔍 Debug mode is now **{state.upper()}**.", delete_after=2)
