...
class Mechanics(commands.Cog):
    ...
    async def meditate(self, ctx: commands.Context):
        ...
        blocked, next_realm, required_ki = await self._is_ki_gain_blocked(user_id, rank, current_ki, max_ki, minor_realm)
        if blocked:
            # Cancel meditation state before sending error
            self.state.cancel_meditation(user_id)
            self.bot.is_meditating.discard(user_id)
            return await ctx.send(
                f"❌ Your Ki has reached the threshold for **{next_realm}** realm.\n"
                f"Use `!breakthrough` to advance before gaining more Ki.\n"
                f"(Current Ki: {current_ki} / Required: {required_ki})",
                ephemeral=True
            )
        # ===================================================

        new_vit = min(max_stats["max_vit"], current_vit + vit_gain)
        new_ki = min(max_ki, current_ki + ki_gain)
    ...
    async def focus(self, ctx: commands.Context):
        ...
        new_ki = min(max_ki, current_ki + FOCUS_KI_GAIN)
